...
const Graphi = () => {
    ...
    const CalculateGraphi = (xl, xr, errorvalue) => {
        if (!isNaN(xl) && !isNaN(xr)) {
            ...
            for (var x = xl; x < xr; x += 0.001) {
                const fx = Error(Equation, x);
                const fxplus = Error(Equation, x + 0.001);
                error = fx;
                if (fx * fxplus < 0) {
                    l = x;
                    r = x + 0.001;
            ...
            while (l <= r && interation < Max) {

                const funcl = Error(Equation, l);
                const funclnice = Error(Equation, l + errorvalue);

                if (isNaN(funcl) & isNaN(funclnice)) {
                    console.error("Error: NaN detected during iteration");
                    return;
                }
                if ((funcl * funclnice) < 0) {
                    break;
                }
            ...
            console.error("Invalid xl or xr");
        }
    };

    const xvalue = data.map(row => row.Xm);
    const yvalue = data.map(row => row.Error);
                    ...
                    <div className='h-[40rem] w-full flex justify-center items-center'>
                        <div className='w-[90%]'>
                            ...
                            <Plot
                                ...
                                layout={{ width: 1200, height: 550, title: 'Graphical Method' }}
                            />
