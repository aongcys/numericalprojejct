...
const Gusseli = () => {
    ...
    const [matrixA, setMatrixA] = useState(Array(size).fill().map(() => Array(2).fill(0)));
    const [matrixB, setMatrixB] = useState(Array(size).fill(0));
    ...
    const calculategausseli = (A, B) => {
        const n = A.length; // หาแถว matrix

        let matrixachange = A.map((row, i) => [...row, B[i]]);

        for (let i = 0; i < n; i++) {
            let maxRow = i;
            for (let k = i + 1; k < n; k++) {
                if (Math.abs(matrixachange[k][i]) > Math.abs(matrixachange[maxRow][i])) {//หา ตัวที่มีค่ามากสุดใน col
                    maxRow = k;
                }
            }

            [matrixachange[i], matrixachange[maxRow]] = [matrixachange[maxRow], matrixachange[i]];//หาแถวที่มีค่ามากที่สุด

            if (matrixachange[i][i] === 0) {
                throw new Error("Matrix is singular or nearly singular");//ข้ามแถวที่ pivot เป็น 0 แล้ว
            }

            for (let k = i + 1; k < n; k++) {
                const factor = matrixachange[k][i] / matrixachange[i][i];//เอาตัวคูณ ที่ จะ ทำให้ pivot เป้น 0
                for (let j = i; j <= n; j++) {
        ...
        for (let i = n - 1; i >= 0; i--) {
            X[i] = matrixachange[i][n] / matrixachange[i][i];//แทนสมการแล้ว นำค่า ใส่ ใน x (แทนกลับ)
            for (let k = i - 1; k >= 0; k--) {
                matrixachange[k][n] -= matrixachange[k][i] * X[i];
            }
        }
        return X;
    };
                        ...
                        <div className='grid grid-flow-col mt-3 gap-4'>
                            <div className='grid grid-flow-col mt-[3%]'>
                                ...
                                <button
                                    ...
                                    onClick={() => {
                                        const result = calculategausseli(matrixA, matrixB);
